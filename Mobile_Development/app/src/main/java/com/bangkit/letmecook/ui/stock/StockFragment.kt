...
//        val userId = FirebaseAuth.getInstance().currentUser?.uid ?: return
    ...
    private fun navigateToEdit(item: InventoryEntity) {
        val fragment = AddStockFragment.newInstance(item)
        parentFragmentManager.beginTransaction()
            .replace(R.id.container, fragment, AddStockFragment::class.java.simpleName)
            .addToBackStack(null)
            .commit()
    }
        ...
        apiService.deleteInventoryItem(id).enqueue(object : Callback<Void> {
            override fun onResponse(call: Call<Void>, response: Response<Void>) {
                if (response.isSuccessful) {
                    ...
                    val updatedList = inventoryAdapter.currentList.filter { it.id_inventory != id }
                    inventoryAdapter.submitList(updatedList)
                } else {
